...
from computeOSF_varyGeom import computeOSF
...
# get the location and list of input files
path= args.roms_file # 'the History File Full Path'
his_filelist=glob.glob(path+'ocean_his_*.nc')
his_filelist.sort()
...
for filetemp in his_filelist:
        his_file=filetemp
        ncROMS = Dataset(his_file, 'r') #read the data of each ocean_his*.nc file
	...
	#zice = variables['draft'][1:-1, 1:-1].T
	...
	for tIndex in range(nTime):
	    ...
	    mask_rho = variables['wetdry_mask_rho'][tIndex, 1:-1, 1:-1].T == 1.0
	    mask_u = variables['wetdry_mask_v'][tIndex, 1:-1, 1:-1].T == 1.0
	    mask_v = variables['wetdry_mask_u'][tIndex, 1:-1, 1:-1].T == 1.0
	    zeta = variables['zeta'][tIndex, 1:-1, 1:-1].T
	    zice = variables['draft'][tIndex,1:-1, 1:-1].T
	    hwater = h + zice
	    ...
	    osfOut[:, 1:-1] = result[nzExtra:, :]
            ...
            print "filling into output index {}".format(fIndex)
	    outVariables['overturningStreamfunction'][fIndex, :, :] = osfOut
	    fIndex=fIndex+1	
            print "new fIndex will be {}".format(fIndex)
